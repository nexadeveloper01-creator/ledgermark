...
> ⚠️ **이메일 인증이 아직 없습니다.** 타인의 이메일로 가입하는 것을 막지 못하고,
> 가입 시 "이미 사용 중인 이메일" 응답으로 가입 여부를 알아낼 수 있습니다(열거).
> 사용성을 위해 명확한 안내 문구를 유지하고 레이트 리밋으로 범위를 제한했으나,
> **운영 전에는 이메일 인증을 도입해야 합니다.** 비밀번호 재설정도 같은 이유로
> 미구현 상태입니다(현재는 운영자가 재발급).
...
- **소비자 이메일 인증과 비밀번호 재설정** — 가입 흐름의 마지막 조각으로 운영 전 필수
